...
enum { SchemeNorm, SchemeSel, SchemeStatus, SchemeLayout, SchemeInfoSel, SchemeInfoNorm }; /* color schemes */
...
drawbar(Monitor *m)
{
	int x, y = borderpx, w, wdelta, sw = 0;
	...
	if ((w = mw - sw - x) > th) {
		if (m->sel) {
			...
			int mid = (m->ww - (int)TEXTW(m->sel->name)) / 2 - x - sp;
			/* make sure name will not overlap on tags even when it is very long */
			mid = mid >= lrpad / 2 ? mid : lrpad / 2;	
			drw_setscheme(drw, scheme[m == selmon ? SchemeInfoSel : SchemeInfoNorm]);
			drw_text(drw, x, y, w - 2 * sp, th, mid + (m->sel->icon ? m->sel->icw + ICONSPACING : 0), m->sel->name, 0);
...
setup(void)
{
	...
	lrpad = drw->fonts->h;
	bh = user_bh ? user_bh : drw->fonts->h + 2 + borderpx * 2;
	updategeom();
	sp = sidepad;
